\documentclass[lang=cn,10pt]{GorgeousnBook}

\title{Leonardo的ICPC模板}
\subtitle{模板用的好，金牌少不了}

\author{列奥那多是勇者}
\institute{狗头吧}
\date{\today}
\version{$0.1.0$}
\bioinfo{邮箱}{azraelzarkli@gmail.com}

\extrainfo{狗头吧在任何时候都是acmer最好的救济}

\setcounter{tocdepth}{3}

\logo{avatar_bust.png}
\cover{cover.jpg}

% % 本文档命令
\usepackage{array}
\newcommand{\ccr}[1]{\makecell{{\color{#1}\rule{1cm}{1cm}}}}
\newtcbox{\hword}[1][orange]{on line, arc = 0.3mm,auto outer arc,colback = #1!12!, colframe = #1!60!,colupper = black, boxsep =0mm, left =0.3mm, right = 0.3mm, top =0.4mm, bottom = 0.4mm,boxrule =0.2mm, bottomrule =0.2mm, toprule = 0.2mm}

% % 修改标题页的橙色带
\definecolor{customcolor}{RGB}{245, 250, 246}
\colorlet{coverlinecolor}{customcolor}
\usepackage{zhlipsum}
\usepackage{longtable}
% ----------
\usepackage{float} %设置图片浮动位置的宏包

\newif\ifshowLink % 是否显示题目链接的变量
\showLinktrue % 电子版最好设为true显示，打印版本应当设为false
% \showLinkfalse

\begin{document}


\chapterimage{catalog.png}
\maketitle
    \input{chapter/prologue.tex}

\frontmatter
\thispagestyle{fancy}
\tableofcontents

\mainmatter
\partsimage{part_0.png}
\part{STL}
\input{chapter/0_stl.tex}

\partsimage{part_6.png}
\part{数据结构}
\input{chapter/1_data_structure.tex}

\partsimage{part_1.png}
\part{基础算法}
\input{chapter/2_basic_algo.tex}

\partsimage{part_9.png}
\part{进阶算法}
\input{chapter/3_graph.tex}
% \input{chapter/2_graph.tex} 进阶部分比较多，每章分开来



\partsimage{part_3.png}
\part{附录}
\input{chapter/appendix.tex}

\end{document}